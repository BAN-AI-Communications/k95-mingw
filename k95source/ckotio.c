...
chkexp( void )
{
...
#ifdef SESLIMIT
    extern int seslimit;            /* Session Time Limit */
#endif /* SESLIMIT */

    memset( &expires, 0, sizeof(struct tm) ) ;
    expires.tm_mday = expday ;
    expires.tm_mon = expmon ;
    expires.tm_year = expyear ;
    etime = mktime( &expires ) + 24 * 60 * 60;
    ltime = time(NULL);

    colorcmd ^= 0x8 ;
    dsn = isregistered(NULL) ;
    ...
    printf("\n NOTICE:\n");
    printf(
"   This software is pre-release and is to be used for testing\n");
        printf(
"   purposes only.  Pre-release versions of Kermit-95 are updated\n");
        printf(
"   on a very frequent basis (several times a week.)  If you find\n");
        printf(
"   a bug and more than a week has transpired since the date listed\n");
        printf(
"   above, please look for an updated version before reporting it.\n\n");
        printf(
"   UNDER NO CIRCUMSTANCES IS THIS PRE-RELEASE SOFTWARE TO BE\n");
        printf(
"   UPLOADED FOR DISTRIBUTION TO ANY SITE OTHER THAN:\n");
        printf(
"        ftp://ftp.kermit-project.org/\n\n");
    }
#endif /* BETATEST */
    ...
    if ( !dsn || dsn < 100 ) {
        unsigned long now_t = time(NULL);

#ifdef COMMENT
        if ( downloaded ) {
            etime = regtime(NULL) + 60*60*24*21;
            memcpy(&expires,localtime(&etime),sizeof(struct tm));
            expday = expires.tm_mday;
            expmon = expires.tm_mon;
            expyear = expires.tm_year;
            expired = (now_t < regtime(NULL) ||
                        now_t > etime);
        }
#endif /* COMMENT */
        ...
        if (!quiet)
            printf(
"   This copy will expire on %d/%d/%d.\n\n",expmon+1,expday,1900+expyear);
        if ( !downloaded && ltime > etime ) {
#ifdef BETADEBUG
            if ( !quiet ) {
                printf(
"   This Beta Test release copy has expired.\n");
                printf(
"   Please retrieve a newer release from ftp://kermit.columbia.edu.\n\n");
            }
#else /* BETADEBUG */
            if ( dsn ) {
                if ( !quiet ) {
                    printf(
"   This DEMO copy of Kermit has expired.\n");
                    printf(
"   Please contact Kermit Orders at kermit-orders@columbia.edu or read the\n");
                    printf(
"   Kermit 95 Web Page at http://www.columbia.edu/kermit/k95.html\n");
                    printf(
"   to continue use of program.\n");
                }
            } else {
                if ( !quiet )
                    printf( 
"   This copy of Kermit 95 must be registered to continue use.\n\n");
            }
#endif /* BETADEBUG */
            for ( i=0;i<3;i++ ) {
                bleep(BP_FAIL);
            ...
            if ( !quiet ) {
                printf(
"   This is a DEMO of version of Kermit 95 designed to allow you to\n");
                printf(
"   experience the full feature set of the program.  This DEMO has a per\n");
                printf(
"   session time limit of 15 minutes.  This provides enough time to test\n");
                printf(
"   the capabilities of the program and ensure compatibility with your\n");
                printf(
"   existing systems.  To purchase a full release of Kermit 95 please\n");
                printf(
"   contact Kermit Orders at kermit-orders@columbia.edu or read the\n");
                printf(
"   Kermit 95 Web Page at http://www.columbia.edu/kermit/k95.html\n");
                printf("\n\n");
            }
...
shoreg( void )
{
    ...
    colorcmd ^= 0x8 ;
    dsn = isregistered(NULL) ;
    /* add K95-00001123108-1.1 to the list below. */

    if ( dsn == 91340 || dsn == 23567 || dsn == 12496 ) {
        if ( !quiet )
            printf(" THIS COPY OF KERMIT SOFTWARE HAS BEEN PIRATED.\n");
        for ( ;; ) {
            bleep( BP_WARN ) ;
            sleep(1);
        }
         ...
         )
    {
        ...
        bleep( BP_WARN ) ;
        if ( !quiet ) {
            printf(" THIS COPY OF KERMIT SOFTWARE IS NOT REGISTERED.\n");
            printf(
" This software must be installed from the original diskettes using the SETUP\n");
            printf(
